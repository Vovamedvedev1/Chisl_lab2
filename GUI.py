...
class GUI:
    def __init__(self, root, fig):
        self.root = root
        self.root.title("Интерполяция полиномом Лагранжа")
        ...
        self.calculate_btn.grid(column=8, row=0, padx=(10, 0), sticky=tk.EW)

        self.error_label = ttk.Label(input_frame, text="", foreground="red",
                                    font=('Arial', 11), wraplength=300, justify=tk.LEFT)
        self.error_label.grid(column=9, row=0, padx=(20, 0), sticky=tk.W)

        for i in range(10):  
            input_frame.columnconfigure(i, weight=1 if i < 9 else 2)
